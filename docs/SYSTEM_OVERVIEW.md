...
| 테스트 | 63 파일 / 658 test, CI(GitHub Actions) 최근 실행 전부 success |
...
- **테스트**: `tests/unit/` 63 파일 / 658 test. LLM·외부 API는 mock(`respx`), 실호출 금지.
...
2. **왼쪽 꼬리** (평가 프레임 정비는 §13.4~13.6으로 완료) — 분포를 보니 방향 적중률은
   51.9%인데 평균이 −2.57%다. 중앙값은 +0.14%로 0 근처 — **틀릴 때 더 크게 잃는다.**
   손절 폭·포지션 사이징·손실 구간 청산 규칙이 다음 검토 대상이다.
   신호 방향을 더 손보는 것보다 우선순위가 높을 수 있다.
