...
다만 표본의 93%가 30분 주기 스캔이라 **서로 독립이 아니다** — 아직 성능 결론은 이르다.
...
> 2026-09-10 평가 큐 수리(§13.1) 후 소급 평가를 마친 값이다. 그 이전에 이 문서가 담고 있던
> 숫자(win_rate 34.4%, 표본 812건)는 **7월 발행분만으로 계산된 구 로직 측정치**였다.

`/signal-accuracy` (7일 horizon, 180일) 실측:

| 지표 | 값 |
|---|---|
| 평가 완료 | **4,272건** (전체 5,018 중 85%, 대기 0, 종결 53) |
| 평가된 발행 기간 | 2026-07-06 ~ 09-03 |
| win / loss / neutral | 1,918 / 1,076 / 1,278 — win_rate **44.9%** |
| 14일 / 30일 평가 | 3,468건 / 1,581건 (수리 전 237 / 0) |
| 칼리브레이터 표본 | 812 → **4,272** |

로직 개편(2026-08-06) 전후를 갈라 보면 성격이 완전히 다르다. `signed` 는 방향 보정
기대값(매수는 +수익률, 매도는 −수익률)으로, 원시 평균수익률과 달리 **방향을 반영한 값**이다.

| 구간 | n | win% (±2% 기준) | 방향 적중% | signed 평균(7일) | signed 중앙값 |
|---|---:|---:|---:|---:|---:|
| 구 로직 (~08-05, KOSPI 급락 국면) | 1,242 | 25.0 | 37.4 | **−4.26%** | −1.28% |
| 신 로직 (08-06~) | 3,030 | 53.1 | 76.5 | **+2.31%** | +2.36% |
| 신 로직 14일 | 2,252 | 69.7 | 76.8 | +3.83% | +3.80% |
| 신 로직 30일 | 365 | 62.2 | 64.9 | +9.40% | +9.22% |
| 신 로직, **scan_agent 제외** | 218 | — | 54.6 | **+0.29%** | +0.22% |

**이 숫자를 읽을 때 반드시 붙어야 하는 단서**:

1. **표본이 독립이 아니다.** 신 로직 3,030건 중 2,812건(93%)이 30분 주기 스캔이다.
   같은 종목이 하루 48회 기록되고 7일 horizon끼리도 겹치므로, 유효 표본은
   7종목 × 약 66일 = 수백 건 수준으로 봐야 한다. 신뢰구간을 계산할 수 있는 형태가 아니다.
2. **일일 멀티에이전트 경로만 보면 +0.29%로 거의 0이다** (n=218). 즉 위의 +2.31%는
   대부분 30분 스캔 경로가 만든 값이고, 리포트·주문이 쓰는 경로의 성능은 아직 미지수다.
3. `/signal-accuracy`의 `avg_return_pct`(−0.352%)는 **방향을 반영하지 않은 원시 수익률**이다.
   매도 신호가 맞을수록 이 값이 내려간다 — 성능 지표로 읽으면 안 된다 (§15-2 참조).
4. win 정의는 7일 수익률이 신호 방향으로 **±2% 이상** 움직인 경우다. ±2% 이내는 neutral로
   분리되므로 win_rate는 승률이 아니다.
5. 53건(`057050.KS`)은 시세 소스에 심볼이 없어 **종결(unresolved)** 처리됐다. 통계에 들어가지
   않으며, 그 티커로 신호가 적립된 것 자체가 상류(티커 접미사 해석)의 별건 결함이다.
...
1. **표본 독립성** (측정 루프 자체는 2026-09-10 복구 완료, §13.1) — 현재 표본의 93%가
   30분 주기 스캔이라 서로 겹친다. 종목·날짜 단위로 어떻게 중복을 제거하고 유효 표본을
   정의할지가 이제 병목이다. 30분 스캔 신호를 애초에 `signal_outcomes`에 전량 적립하는 것이
   맞는지도 함께 봐야 한다 (하루 48건 × 7종목).
