...
목적지 후보 실측: `hsptest-macstudio`(문서상 듀얼 노드 파트너)는 **5일째 offline**,
`testmacstudio2-macstudio`·`parkhongnas` 는 SSH 응답. 어디에 둘지는 운영 판단이다.

##### 남은 한계 (문서에 명시)

- 오프사이트 복제 기능은 있으나 **목적지가 아직 미설정**이다 (`OFFSITE_BACKUP_DEST=`).
  설정 전까지 백업은 같은 디스크에만 있다 — `/ops/backups` 가 그 사실을 보고한다
