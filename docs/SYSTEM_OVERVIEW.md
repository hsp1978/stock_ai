...
##### 2026-09-16 정정 — 위 결론은 무효다

§13.9p 의 교착이 풀린 뒤(`systemctl restart ollama`) 같은 조건으로 재측정했다.

| 조건 | 결과 |
|---|---|
| 단독 1요청 | 128 tok, **64.39 tok/s**, 2.12s |
| 동시 2요청 | 64.2 / 64.41 tok/s — **요청당 저하 0**, 벽시계 4.16s (큐잉 직렬화) |
| VRAM | 9,488 / 12,227 MiB — 여유 **2.7GB** |
| Mac 대조 (qwen2.5:32b) | 9.63 tok/s → **RTX 가 6.7배 빠르다** |

내가 적은 근본 원인 — "여유 1.4GB 라 동시 2요청의 KV 캐시를 감당하지 못한다" — 은
세 항목 모두 틀렸다. 여유는 2.7GB 이고, 동시 요청은 깨끗이 직렬화되며, 요청당
처리량은 변하지 않는다.

실제로는 **분산 측정 자체가 교착되어 가던 노드를 잰 것**이다. Quant 320.1초·Risk
279.1초는 실효 ~2 tok/s 수준으로 정상 노드의 값이 아니다. 나는 같은 문서에 "측정
직후 RTX 는 8토큰 요청조차 90초 내에 끝내지 못했다"고 적어두고도, 그 앞의 측정값은
유효한 것으로 읽었다 — **고장난 노드에서 잰 수치로 설계 결정을 내린 것**이다.

이것이 §13 의 실패 은폐 계열과 같은 형태다: 관측은 남겼는데 그 관측이 **직전
측정을 무효화한다는 함의**를 읽지 않았다. 교훈은 "이상을 발견하면 그 이상이 이미
끝난 측정에 소급 적용되는지 먼저 따진다"이다.

**따라서 2:2 분산의 가부는 아직 미결이다.** 정상 RTX 기준으로 다시 재야 한다.
...
#### 13.9q 프로브가 바쁜 노드를 고장으로 읽었다 (2026-09-16, §13.9p 회귀)

2:2 분산(§13.9o 정정)을 배포한 직후 `/health` 가 다시 `runtime: unusable,
generation: stalled` 를 보고했다. 그런데 이번엔 GPU 가 **98% / 250W 로 일하고
있었다.** 30분 `watchlist_scan` 이 새 매핑으로 RTX 를 쓰는 중이었다.

두 노드 모두 `OLLAMA_NUM_PARALLEL=1` 로 직렬 처리한다. 바쁜 노드에 보낸 1토큰
프로브는 큐 뒤에 줄을 서고 예산(20초)을 넘긴다 — **정상 노드가 `stalled` 로
보고된다.**

Mac 이면 더 나쁘다. `is_mac_studio_available()` 이 False 가 되어 **바쁠 때
정확히 라우팅에서 빠진다.** 부하가 노드를 제거하고, 제거가 남은 노드의 부하를
키우는 되먹임이다. 배치가 가장 바쁜 순간에 노드를 하나 잃는다.

##### 수정 — 처리 중이면 건너뛴다

`node_slot` 이 in-flight 를 노드별로 집계하고 있다 (배치·스캔 양쪽 경로가 쓴다:
`multi_agent._call_ollama`, `llm/router.py`). 그 노드에 우리 요청이 돌고 있으면
프로브를 **보내지 않는다** — `skipped` / `reason: node_busy`.

근거는 두 가지다:

- **처리 중이라는 사실이 1토큰 합성 호출보다 강한 증거다.** 요청이 돌고 있으면
  생성 경로는 살아 있다. 합성 호출은 그보다 약한 확인을 하면서 큐만 늘린다
- 건너뛰면 **프로브가 부하를 더하지 않는다**. §13.9p 에서 이미 "유휴 노드에
  보내면 모델 로드를 유발한다"는 이유로 idle 을 건너뛰게 했는데, 같은 원칙이
  바쁜 노드에도 적용된다

교착 탐지는 그대로다 — 우리 요청이 없는데 1토큰도 안 나오면 여전히 `stalled` 다.
§13.9p 의 실제 사례가 정확히 그 상태였다 (GPU util 0%, 8.8W, 아무것도 안 돌고
있는데 생성 불가).

##### 이 회귀의 성격

§13.9p 는 "적재됐다"를 "쓸 수 있다"로 읽는 것을 고쳤다. 그 수정이 이번엔
"바쁘다"를 "고장났다"로 읽었다. **두 번 다 프로브가 관측할 수 없는 상태를
단정한 것**이다. 프로브는 모르는 것을 모른다고 말해야 한다 — `skipped` 가
`ok` 도 `stalled` 도 아닌 별도 상태인 이유다.

#### 13.9r 스캔 경로가 어느 집계에도 잡히지 않았다 (2026-09-16, §13.9q 의 근인)

§13.9q 를 배포하고도 `/health` 가 다시 `unusable` 을 보고했다. 30분 스캔이 도는
동안 in-flight 집계는 `{}` 였다.

`ChartAnalysisAgent._run_ollama_agent`(`analysis_tools.py`)가 Ollama 를 **직접**
쳤다 — `node_slot` 을 거치지 않는 유일한 LLM 경로였다. 배치(`multi_agent`)와
라우터(`llm/router.py`)는 둘 다 슬롯을 쓰는데 이 경로만 빠져 있었다.

두 가지가 깨져 있었다:

1. **노드 동시 요청 제한이 이 경로에만 적용되지 않았다.** `RTX_5070_MAX_INFLIGHT`
   는 스캔에 대해 한 번도 효력이 없었다
2. **헬스 프로브가 "바쁘다"를 알 수 없었다.** §13.9q 의 건너뛰기는 `node_slot`
   집계를 읽는데, 그 집계가 비어 있으니 발동하지 않는다. 스캔 중 프로브가 큐
   뒤에서 20초 타임아웃하고 정상 노드를 `unusable` 로 보고했다

##### 수정

Ollama 호출부를 `_local_node_slot()` 안으로 옮겼다. **슬롯을 못 잡아도
진행한다** — 목적은 부하를 집계에 드러내는 것이지 막는 것이 아니다. 이 경로는
원래 제한이 없었으므로, 여기서 막으면 동작이 바뀐다. `dual_node_config` 를 못
불러오면 아무것도 하지 않고 종전대로 돈다.

##### 교훈

§13.9q 의 수정은 **옳았지만 작동하지 않았다.** 근거로 삼은 집계가 정작 문제의
경로를 담고 있지 않았기 때문이다. 관측에 기반한 판단을 넣을 때는 **그 관측이
대상 경로를 실제로 덮는지** 먼저 확인해야 한다. "집계가 있다"와 "집계가 이
경로를 본다"는 다른 말이다.
...
#### 13.9t 노드 분산의 한계 — 속도 차이는 모델 차이다 (2026-09-17)

§13.9s 수정 후 배치. **성공 7 / 실패 0, 662초** (776 → 662, 최초 1,117).
검증 실패 **17건 → 0건**.

| 에이전트 | 노드 | 평균 | 직전 | 변화 |
|---|---|---|---|---|
| Risk Manager | Mac | 68.0s | 55.7s | 1.22x |
| Quant Analyst | Mac | 63.1s | 74.2s | 0.85x |
| Event Analyst | Gemini | 25.6s | 7.3s | 3.51x |
| Geopolitical | Gemini | 15.8s | 13.9s | 1.13x |
| Value Investor | Gemini | 9.0s | 6.4s | 1.40x |
| **ML Specialist** | RTX | **6.3s** | 108.4s | **0.06x** |
| **Technical Analyst** | RTX | **3.5s** | 22.1s | **0.16x** |

Ollama 4개 합 260.5 → **141.0초**. 병목이 **RTX 에서 Mac 으로 옮겨갔다** —
RTX 는 종목당 10초만 쓰고 논다.

##### 더 옮기면 어떻게 되나 (A/B, Gemini 미사용)

첫 측정은 무효였다. `RTX_5070_MAX_INFLIGHT` 기본값이 2라 3번째 에이전트가
슬롯을 못 잡고 **Mac 으로 샜다** (`overload_count=7`). 3:1 을 잰 게 아니라
2:2 를 다시 잰 것이었다. 동시성을 4로 올려 다시 쟀다 (overload 0):

| 배치 | 벽시계 | 대비 |
|---|---|---|
| A 2:2 (현재) | 66.5s | — |
| B 3:1 (Quant→RTX) | 33.8s | 1.97x |
| **D 4:0 (전부 RTX)** | **12.7s** | **5.22x** |

RTX 는 4개를 직렬로 12.7초에 끝낸다. Mac 은 **1개가 33.8초**다.

##### 그런데 신호가 바뀐다

A/B 에서 Mac 의 Quant·Risk 가 `neutral` 인데 RTX 로 옮기면 `buy` 였다.
같은 입력으로 노드별 5회씩 다시 쟀다:

| 에이전트 | 노드·모델 | 신호 분포 | 평균 신뢰도 |
|---|---|---|---|
| Quant Analyst | Mac qwen2.5:32b | neutral 4 / buy 1 | **5.0** |
| Quant Analyst | RTX qwen3:14b | **buy 5** | **6.9** |
| Risk Manager | Mac qwen2.5:32b | neutral 4 / buy 1 | **5.1** |
| Risk Manager | RTX qwen3:14b | **buy 5** | **6.5** |

**분포가 겹치지 않는다.** qwen3:14b 가 체계적으로 더 낙관적이고 더 확신한다.

신뢰도 차이는 장식이 아니다. `enhanced_decision_maker.STRENGTH_CAP_CONFIDENCE
= 5.0` 이라 그 아래면 강도 라벨이 절하된다. Mac 은 경계선(5.0~5.1)에 있고
RTX 는 상한이 풀리는 쪽(6.5~6.9)이다 — **같은 근거로 더 센 신호가 나온다.**

##### 결론 — 매핑을 바꾸지 않는다

속도 차이의 정체는 노드가 아니라 **모델**이다 (14B vs 32B). 5.22배를 얻으려면
판단 주체를 바꿔야 하고, 그건 성능 튜닝이 아니라 **시스템 동작 변경**이다.

어느 쪽이 맞는지는 모른다. `signal_outcomes` 를 소스별로 쌓아야 한다.
§9.2("측정 가능성 > 최적화")에 따라 그때까지 속도를 이유로 더 옮기지 않는다.

**이미 옮긴 둘(Technical·ML)에도 같은 효과가 적용된다.** §13.9o 정정에서
"품질 영향은 미측정"이라고만 적었는데, 이제 방향은 측정됐다 — 낙관·고신뢰
쪽이다. 2026-09-16 배치부터 적용 중이므로 그 이후 표본을 별도로 봐야 한다.

##### 남은 것

- Event Analyst(Gemini)가 7.3 → 25.6초(최대 118.5초)로 튀었다. 제 변경과
  무관한 경로이고 outlier 1건이 평균을 끌어올린 형태다. **원인 미확인**
- `RTX_5070_MAX_INFLIGHT` 기본 2는 3:1 이상을 막는다. 매핑을 바꾸려면 이
  값도 함께 올려야 한다 — 안 그러면 조용히 Mac 으로 샌다
