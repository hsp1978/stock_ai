...
사이드바는 **페이지 이동만** 담당한다. 17개 페이지가 3그룹으로 묶여 있고, 그룹
헤더(`▾ ANALYSIS`)를 누르면 접힌다. 접힘 상태는 유지된다.

| 그룹 | 페이지 |
|---|---|
| **ANALYSIS** | Home · Dashboard · Detail · Multi-Agent · Quant Indicators · Screener · Signal Accuracy · ML Predict · Backtest |
| **OPERATIONS** | Scan Log · System Monitor · History · Ranking |
| **TRADING** | Trading · Virtual Trade · Paper Trade · Portfolio |

선택된 페이지는 좌측에 파란 세로 바가 붙는다.

### 3.2 상단 커맨드바 — 조작

스캔·GPU·시스템 제어는 **화면 상단**에 있다 (사이드바가 아니다).

| 컨트롤 | 설명 |
|---|---|
| 검색창 | 티커 또는 한글 종목명 입력 |
| **Scan** | 입력한 종목 1건 즉시 분석 |
| **Scan All** | Watchlist 전체 병렬 스캔 |
| **GPU** ⌄ | GPU 일시 해제 / 연장 / 즉시 복구 (아래 3.3) |
| **시스템** ⌄ | Agent 상태, 캐시·스캔 수, 모델, 임계값, Restart Agent |

### 3.3 GPU 일시 해제

다른 서비스에 GPU를 잠깐 양보할 때 쓴다. `GPU ⌄` → 시간(30/60/90/120분) 선택
→ `⏸ GPU 사용 중지`.

- 해제 중에는 스케줄 스캔·V2 배치가 **건너뛰어지고** 수동 스캔은 409로 거절된다.
  하나라도 열어두면 모델이 다시 적재되어 해제 목적이 무너지기 때문이다.
- **지정 시간이 지나면 자동 복귀**한다. 복구를 잊어도 서비스가 멈춘 채 방치되지
  않는다. 상한은 4시간.
- 작업이 길어지면 `⏱ 연장`(+15/+30/+60분)으로 남은 시간에 더할 수 있다.
- 진행 중인 스캔이 있으면 VRAM이 즉시 반환되지 않을 수 있다. 이 경우 응답과
  `/health`의 `gpu_pause.unloaded`에 실패가 표시된다.
...
**시장 지수 타일** (12개, 4열):
- 지수: S&P 500 · NASDAQ · DOW · KOSPI · KOSDAQ
- 환율(**원화 기준**): 원/달러 · 원/100엔 · 원/위안
- 원자재: Gold · Silver · Copper · Nat Gas

각 타일은 `라벨 / 가격 / 등락률` 3단이고, 방향은 색과 함께 ▲▼ 기호로도 표시된다.
**타일을 클릭하면 아래에 추이 차트**가 펼쳐진다 — 기간은 1개월~5년 중 선택하고,
기간 등락률·최고·최저가 함께 나온다. 같은 타일을 다시 누르면 접힌다.

> 환율은 국내 관점(“1달러가 몇 원”)으로 표기한다. 엔은 국내 관례대로 100엔 기준.
> yfinance의 원화 직접 페어는 데이터가 성겨(`CNYKRW=X`는 1개월에 1봉) USD 페어에서
> 교차 계산한다.

**분석 요약**: Total / Buy / Sell / Hold / US / KR 6칸 분할 행.
스캔 결과가 없으면 0을 반복하지 않고 “스캔 없음”으로 표시된다.

**System Status**: Agent 상태 · 캐시 결과 수 · 총 스캔 수 · 모델명

**Watchlist 섹션**:
- 종목 칩은 `KR 005930.KS` 형태로 **티커 우선 + 시장 배지**, 회사명은 툴팁
- 필터: 전체 / US / KR
- 빠른 추가 입력창 (영문/한글/6자리 코드 모두 인식)
- **삭제**: 복수 선택 후 `🗑 선택 N개 삭제`, 또는 확인 체크박스를 켠 뒤 `🗑 전체 삭제`
  (삭제 실패 시 화면에 오류가 표시된다 — 조용히 되살아나지 않는다)
