...
### 2026-09 평가 프레임 정비 + 종목 확대 (완료)
- 시장 대비 초과수익(벤치마크 컬럼), ECE 홀드아웃(out-of-time), 평가 horizon ↔ 의도
  보유기간 정합(swing 10일 → 대표 14일), ±2% 밴드를 대표 지표에서 제거(방향 적중률 +
  분포), 역행폭 기록 + 손절 시뮬레이션
...
- `sampling.rows_raw`/`collapse_ratio`/`dominant_source_share_pct` 동시 보고 — 원시 수를 숨기지 않는다
- `llm_calibrator`(ECE·isotonic), `ic_ensemble`(소스별 IC)도 같은 표본 단위 사용
- 효과: 같은 데이터 승률 44.9%(원시 4,272행) → 33.7%(표본 1,223건, 독립 블록 378)

### 2026-09 평가 큐 수리 (완료)
- 사후 평가 후보 선정을 '도래한 horizon, 오래된 순'으로 교정 (종전: 최신순 500건 → 평가 0건)
- 창/배치 config화 (`SIGNAL_EVAL_DAYS_BACK=90`, `SIGNAL_EVAL_BATCH_LIMIT=2000`)
- 시세는 티커별 1회 배치 조회. 시세 없는 심볼은 `eval_state='unresolved'`로 종결
- 잔량(`pending_due`)이 결과·검증 잡 status에 실린다 — 무동작 런이 completed로 안 보인다
- 소급 평가 4,245건 완료: return_7d 812→4,272 / 14d 237→3,468 / 30d 0→1,581
...
신호 로직 연속 변경(~08-05).
표본은 2026-09-10 소급 평가로 생겼고 표본 단위(`ticker_day`)까지 적용했다.
현재 로직 실측: 방향보정 기대값 **+0.72%(독립 블록 117)**, 리포트·주문이 쓰는
일일 멀티에이전트 경로만 보면 **+0.29%(블록 100)**. 30분 스캔만 보면 블록 17개라
통계가 아니다. 승률 ±5%p CI에는 수백 건이 필요하므로 **방향성 점검 용도**로만 쓸 것.
